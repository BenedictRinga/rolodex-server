...
- The home page centers the LOOPKEEPER INBOX (tabs: Assistant | Loops | Reminders — the Assistant tab was called Chat until 2026-08-28). The Loops tab is the Open Loops Inbox: capture a loop in one sentence, get a context packet + ready draft (tones: short/honest/light/formal), decide via Send draft / Wait until / Drop, send one-tap into WhatsApp/SMS/Email/LinkedIn/Telegram (2026-08-28 build 132: Telegram joined; the card's send menu also carries X/Snapchat/TikTok opens and a Copy-draft button), receive receipts, and see "Today's 3". Waiting-on-you and waiting-on-them are separate piles; dropping with dignity counts as closing. (2026-08-30, build 157: the Loops tab's DEFAULT surface is now the SEND WALK — four slides, one decision each, built for the chronic prevaricator: Who (the engine's picks of loops already waiting, plus the full device contacts deck) → What (chips read from card truth — an owed reply, a promise, a check-in — or one line in their own words) → Words (the engine's draft, four tones, polish, or their own words) → One tap (WhatsApp, a call door with a 20-second plan, SMS/email under More ways, and a universal Copy door; the words ALWAYS ride the clipboard). Sending IS the close: a completion chime, an Off-your-mind slide, then Next one. The full Open Loops Inbox described above is still there, packed behind the walk's quiet "I'm good" — and the shelf answers with "Smooth" to return. No cues, no labels; the discovery is the toggle. If a user says they cannot find the old inbox, tell them: tap "I'm good" at the foot of the walk.)
