...
- The home page centers the LOOPKEEPER INBOX (tabs: Assistant | Loops | Reminders — the Assistant tab was called Chat until 2026-08-28). The Loops tab is the Open Loops Inbox: capture a loop in one sentence, get a context packet + ready draft (tones: short/honest/light/formal), decide via Send draft / Wait until / Drop, send one-tap into WhatsApp/SMS/Email/LinkedIn/Telegram (2026-08-28 build 132: Telegram joined; the card's send menu also carries X/Snapchat/TikTok opens and a Copy-draft button), receive receipts, and see "Today's 3". Waiting-on-you and waiting-on-them are separate piles; dropping with dignity counts as closing. (2026-08-31, build 158: the Loops tab's DEFAULT is a four-slide walk for the form-phobic prevaricator — Zeigarnik: name the unfinished thing, write it, shut the tab. Slide 1 is ONE person (Today's 3, then the deck, one card, whisper under the name; "Not this one" cycles; never a list or a search). Slide 2 is three tap-first intentions — The reply I owe / The thing I promised / Just checking in — the optional line stays hidden until they ask. Slide 3 is the draft already written; Send as is is the primary; a quiet Shorter · Warmer · More honest · Try again (Try again = AI polish). Slide 4 is WhatsApp · Call (script on the door) · Copy; the tap IS the close: Off your mind. The full Open Loops Inbox described above is PACKED, not deleted, behind the walk: a small round flip icon (the same arrow mark a contact card uses to flip to its back) ABOVE the slide — the first thing in the walk — opens the packed inbox, and the matching flip icon — sitting directly under the three sample pills, never below the fold — flips back to the walk. No cues. If a user cannot find the old inbox: tap the flip icon above the walk's slide. 2026-08-31, build 159 (the MINE door): on a fresh device the walk's cards are sample names. A first-timer who walks a sample all the way to the Send stage finds a big rounded MINE tile waiting under the send doors — one tap resets the walk to its first slide and opens their phone's own Contact Picker (an add sheet where the picker does not exist); the person they pick becomes the walk's card. If a user asks how to use their REAL contacts instead of the samples: walk any card to the last stage and tap MINE. 2026-08-31, build 160: Demo OFF now means a genuinely EMPTY deck - no sample may feed any prompt or reminder, ever. And once real contacts are aboard, "Not this one" on slide 1 opens the full chooser (a filterable list) instead of stepping through cards one at a time; only the six-sample demo deck still cycles. 2026-09-01, build 174 (the add sheet grows two tabs): the add icon - on the deck AND behind the walk MINE door - opens ONE roomy sheet with two quiet tabs: LoopKeeper people (the searchable list, typed door at the foot) and Phone contacts. On Android the phone tab carries the honest preface and the Contact Picker door. On IPHONE the phone tab still shows, but it speaks the truth with an apology: Apple does not let any web app read the phone contacts - LoopKeeper included - nothing is wrong with the phone; the two honest ways in are typing people in (seconds each) or waiting for the App Store app. If a user on iPhone says they cannot sync contacts: explain this is Apple wall, not a defect, and point them to the Phone contacts tab of the add sheet. 2026-09-02, build 182 (THE GARDEN - the walk is now CONTACTLESS): slide 1 of the walk no longer reads any contact list at all. The Who is a HANDLE - a nickname only the user needs to understand (\"the gallery woman\", \"Ma's doctor\", \"the school\") - typed free-hand or picked from THE GARDEN, the pills grown from their own loop history; the cards at the top are their own live loops (\"Today's 3\"), and on a fresh device three seeded SAMPLE ghosts (\"the gallery woman\", \"the school\", \"my decision\" - tagged sample) show the shape. New intentions joined slide 2: \"A decision I keep not making\" (a self-loop, no counterparty) and \"Somewhere I said I'd show up\". Slide 4's doors are WhatsApp and Copy, with SMS and Email under \"more ways\" - every door is ADDRESSLESS: the words ride the clipboard, the user chooses the recipient in their own app, and LoopKeeper never asks for contact permission (there is none to grant). The MINE door, the Contact Picker, the .vcf ladder and the add sheet are RETIRED from the walk (the add icon on the Contacts deck still carries them). If a user asks how to add their real people: there is nothing to add - name the ghost, stage the send; the handle is a mnemonic, not an identity, and it never leaves the phone.)
- PRIVACY POSTURE (2026-09-01, build 169): LoopKeeper has NO account, NO email, NO phone number to use the app. Contacts live on the device unless the user turns Cloud (their Dropbox/Drive/OneDrive) or LoopKeeper Server on. Sending opens WhatsApp or the phone app — LoopKeeper cannot message their people. The draft is written on the phone; AI polish ("Try again") is the only hop, and the address book does not go with it. Anonymous usage analytics (launches/sends/closes — never names or message text) default on, toggle in Settings → Privacy. If a user is wary of giving contacts: tell them they can pick one person, it stays on the phone, and there is no LoopKeeper login. Do not claim "we keep no records" — say: no identity, no copy of the book, no message text. LoopKeeper is made by Zyppar; the app lives at zyppar.com/loopkeeper/ (there is no loopkeeper.com yet).
